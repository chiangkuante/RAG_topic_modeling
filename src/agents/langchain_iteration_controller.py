"""
LangChain-based Iteration Controller
使用LangChain构建的主题建模迭代控制器
"""
...
from .langchain_reasoning_agent import LangChainTopicReasoningAgent, TopicResult
from .langchain_evaluation_agent import LangChainTopicEvaluationAgent, EvaluationResult
...
class IterationState:
    """迭代狀態數據類"""
    iteration_count: int
    current_topics: Optional[TopicResult]
...
class LangChainTopicModelingController:
    """基於LangChain的主題建模迭代控制器"""
    ...
    def __init__(self, 
        ...
        self.reasoning_agent = LangChainTopicReasoningAgent(llm_manager)
        self.evaluation_agent = LangChainTopicEvaluationAgent(vector_database, quality_threshold, llm_manager)
    ...
    def run_topic_modeling(self, 
        ...
        logger.info(f"開始LangChain主題建模迭代流程，最大迭代次數: {self.max_iterations}")
        self.reset_state()
        
        # 檢查LLM是否已配置
        try:
            self.llm_manager.get_active_llm()
        except ValueError as e:
            return self._create_error_result(f"LLM未配置: {e}")
        ...
        while not self._should_stop():
            ...
            try:
                ...
                if self.state.current_topics is None:  # 如果連初次生成都失敗
                    return self._create_error_result(f"主題生成失敗: {e}")
                break
    ...
    def _prepare_evaluation_context(self, retrieval_result: RetrievalResult) -> str:
        ...
        if retrieval_result.documents:
            ...
            context_parts.extend([f"- {doc[:150]}..." for doc in retrieval_result.documents[:3]])
    ...
    def _create_final_result(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            'metadata': {
                ...
                'llm_provider': self.llm_manager.config.provider.value if self.llm_manager.config else 'unknown',
                'framework': 'langchain'
            }
        }
        
        logger.info(f"LangChain主題建模完成: {len(best_topics.topics)} 個主題，最終分數: {self.state.best_score:.2f}")
        return result
...
class LangChainBatchTopicModeling:
    """基於LangChain的批量主題建模"""
    
    def __init__(self, controller: LangChainTopicModelingController):
    ...
    def run_multiple_rounds(self, 
        ...
        logger.info(f"開始 {num_rounds} 輪LangChain批量主題建模")
